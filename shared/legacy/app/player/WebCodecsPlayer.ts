...
export class WebCodecsPlayer extends BaseCanvasBasedPlayer {
    ...
    protected decode(data: Uint8Array): void {
        ...
        const startCodeLength = WebCodecsPlayer.getStartCodeLength(data);
        if (!startCodeLength) {
            return;
        }
        const type = data[startCodeLength] & 31;
        const isIDR = type === NALU.IDR;

        if (type === NALU.SPS) {
            const { codec, width, height } = WebCodecsPlayer.parseSPS(data.subarray(startCodeLength));
    ...
    private static getStartCodeLength(data: Uint8Array): number {
